...
| `style` | コードスタイル変更（フォーマット、インデントなど） | `style: インデントを統一` |
| `refactor` | コードのリファクタリング（機能は変えずに内部構造を改善） | `refactor: 関数名をわかりやすく変更` |
| `test` | テスト関連 | `test: ログイン機能のテストを追加` |
| `chore` | その他の作業（ビルドプロセス、ライブラリ更新など） | `chore: パッケージを更新` |

`style`と`refactor`の違い:
